...
| `PolarisAddons` | `PolarisAddons.dll` | 自定义物品、插件与技能的定义、目录、IoC、状态/Modifier 运行时及原版游戏投影 |
| `PolarisMap` | `PolarisMap.dll` | 当前地图编辑、首版 TMAP/`.pmap` 创建与切换、热重载和调试能力 |
...
| `PolarisAI` | `PolarisAI.dll` | `.pai` 行为树运行时、Actor/NPC 适配、`.pnpc` 自定义 NPC 与热重载 |
...
- `PolarisCore/doc/design`：Game API 的边界与设计说明。
- `PolarisCore/doc/specs`：Game API v3、v4 表格规格。
- `PolarisMagic/doc/design`：PolarisMagic 的设计规范与实现说明。
- `PolarisEvent/doc/design`：PolarisEvent / PEVT 的设计、实施文档和阶段契约。
- `PolarisAI/PolarisAI.Authoring`：可供编辑器复用的 `.pai` / `.pnpc` 文档、序列化与校验项目。
