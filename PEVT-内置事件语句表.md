...
| 语句 | 执行方式 | 返回值 | 参数签名 | 作用 |
| --- | --- | --- | --- | --- |
| `@say` | 同步等待 | 无 | `actorId : string, text : string` | 以指定角色说话；等待文本显示及玩家推进。 |
...
| 语句 | 执行方式 | 返回值 | 参数签名 | 作用 |
| --- | --- | --- | --- | --- |
| `@actor_enter` | 等待／可并行 | 无 | `actorId : string, position : string, appearanceId : string, frames : int` | 设置外观并让角色进入指定语义位置。 |
...
| `@actor_appearance` | 立即 | 无 | `actorId : string, appearanceId : string` | 更换角色立绘、服装或差分组。 |
...
- `position` 使用语义锚点，第一版至少包含 `left`、`center`、`right`、`off_left`、`off_right`。
