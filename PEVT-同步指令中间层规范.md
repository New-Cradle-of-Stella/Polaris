...
| `Resources` | 解析、预载并等待图像、音频、地图与角色资源。 |
...
| `@say` | `Dialogue.SelectSpeaker(actorId)` → `Dialogue.OpenText(text)` → `Dialogue.WaitAdvance()` → `Dialogue.CommitLog()` | 无 |
...
| `@` API | 有序原子方法 | 最终结果 |
| --- | --- | --- |
| `@actor_enter` | `Resources.RequirePortrait(actorId, appearanceId)` → `Portrait.SetAppearance(actorId, appearanceId)` → `Portrait.Place(actorId, position, frames)` | 无 |
| `@actor_exit` | `Portrait.Exit(actorId, frames)` | 无 |
| `@actor_move` | `Portrait.Move(actorId, position, frames)` | 无 |
| `@actor_appearance` | `Resources.RequirePortrait(actorId, appearanceId)` → `Portrait.SetAppearance(actorId, appearanceId)` | 无 |
