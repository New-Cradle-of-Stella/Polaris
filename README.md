...
- **按当前语言取词** —— 按 `PolarisAPI.Game.CurrentLocale`（玩家当前选的游戏语言）匹配对应语言列，匹配不到就用中性值兜底
...
- **Current-language lookup** — matches `PolarisAPI.Game.CurrentLocale` against the per-language columns, falling back to the neutral value when there's no match
